...
class CorrectiveRAGWorkflow(Workflow):
    ...
    @step
    async def scratchpad_decision_making(self, ctx: Context, ev: RefinementEvent) -> RefineOnRepoEvent | RefineOnFolderEvent | StopEvent:
    ...
    @step
    async def folder_refining(self, ctx: Context, ev: RefineOnFolderEvent) -> RetrieveEvent | StopEvent:
        ...
        await ctx.set("raw_links", raw_links)
        # await ctx.set("",)
    ...
    async def parse_links(
        ...
        for chunk in scratchpad:
            links = re.findall(r'https?://[^\s)"]+(?=[)\s,."\'!])?', chunk.page_content)
            link_list += links
        top_5_links = self.best_5_links(link_list, query)
        print(f'Top 5 - {top_5_links}')
        # return LinkParseEvent(links=top_5_links)
        ...
        return StopEvent(result=top_5_links)
